...
class AppLaunchParallax : UIViewController, UIScrollViewDelegate {
    
    var completionHandler: RMParallaxCompletionHandler!
    var items: [AppLaunchParallaxItem]!
    var motion = false
    ...
    */
    required init(items: [AppLaunchParallaxItem], motion: Bool) {
    ...
    func setupRMParallax() {
        ...
        let dismissSize:CGFloat = 36
        self.dismissButton = UIButton(frame: CGRectMake(self.view.frame.size.width / 2.0 - dismissSize*0.5, self.view.frame.size.height - 20.0 - dismissSize*0.5, dismissSize, dismissSize))
        ...
        for (index, item) in self.items.enumerate() {
            ...
            //
            let font = UIFont(name: "PlayfairDisplay-BoldItalic", size: 30) ?? UIFont.systemFontOfSize(30)
            ...
            let textView = UITextView(frame: CGRectMake(5.0, self.view.frame.size.height / 2.0 + 100, rect.size.width, rect.size.height))
            textView.text = item.text
            textView.textColor = UIColor.whiteColor()
            textView.backgroundColor = UIColor.clearColor()
            textView.userInteractionEnabled = false
            imageView.image = item.image
            textView.font = UIFont(name: "PlayfairDisplay-BoldItalic", size: 25) ?? UIFont.systemFontOfSize(25.0)
